...
def _place_rotated_image(ax, image_path, center, span_xy, angle_deg,
                         angle_offset_deg=0.0, zorder=6):
    """Draw `image_path` rotated by (angle_deg + offset) about its centre and
    scaled so the full image spans exactly span_xy (w, h) in data units, placed
    at `center`. Assumes the axes have equal x/y data scale (true here)."""
    from matplotlib.transforms import Affine2D
    img = mpimg.imread(image_path)
    hw, hh = 0.5 * span_xy[0], 0.5 * span_xy[1]
    im = ax.imshow(img, extent=[-hw, hw, -hh, hh], aspect="auto",
                   zorder=zorder, interpolation="bilinear")
    im.set_transform(Affine2D().rotate_deg(angle_deg + angle_offset_deg)
                     .translate(center[0], center[1]) + ax.transData)
    im.set_clip_path(ax.patch)              # keep it inside the panel
    return im
...
def _linear_glyph(ax, window_um, center, angle_deg=0.0, fontsize=12):
    ...
    ax.add_patch(FancyArrowPatch((cx - d[0], cy - d[1]-0.1), (cx + d[0], cy + d[1]-0.1),
                                 arrowstyle="<|-|>", mutation_scale=14,
                                 color="crimson", lw=2.2, zorder=8))
    ax.text(cx, cy + 0.00 * window_um, r"$E_{pol}$", color="crimson",
...
def _radius_annotation(ax, center, radius, angle_deg=-58.0, color="black",
    ...
    ax.text(lab[0]-10, lab[1]+17, rf"$R_T \approx {radius:.0f}\,\mu$m",
...
def plot_with_force_curve(
    ...
    n_markers=6,
    handedness=+1,
    particle_image="particle.png",   # image drawn (rotated) at each path end
    image_span_um=None,              # full-image physical span; None -> particle
    image_angle_offset_deg=0.0,      # add if the image's "front" isn't +x
    linear_pol_angle_deg=0.0,
    window_pad=1.05,           # tight framing -> orbit nearly fills its panel
    lin_height_frac=0.28,      # height of the linear strip as fraction of column
    line_color="#0f54c4",
    line_width=3.5,            # bolder lines add visual weight on a poster
    marker_color="0.45",       # darker grey -> markers carry weight
    tau_color="#1b4965",
    force_color="#e8913a",
    panel_bg="#f7f4ef",        # light tint so panels aren't stark white
    note_linear=None,
    note_circular=None,
    speed_um_s=None,           # if given, shows "v ~ X um/s" in the linear panel
    font_scale=1.5,            # >1 enlarges all ticks/labels for posters
    max_points=4000,
    fig_size=(15.5, 7.0),
    out_path="metavehicle_force_curve.svg",
    dpi=300,
    show=False,
):
    ...
    if np.isscalar(particle_size_um):
        particle_size_um = (float(particle_size_um), float(particle_size_um))
    img_span = image_span_um if image_span_um is not None else particle_size_um
    if np.isscalar(img_span):
        img_span = (float(img_span), float(img_span))
    ...
    left1, gap_x = 0.085, 0.18      # wider gap so the F_x twin axis clears the
    ...
    l1, = ax_curve.plot(phi_deg, tau_curve, color=tau_color, lw=3.0,
                        label=r"$\tau_z/\tau_0$", zorder=3)
    ax_curve.set_xlabel(r"orientation $\varphi$ (deg)")
    ax_curve.set_ylabel(r"$\tau_z/\tau_{max}$", color=tau_color)
    ax_curve.tick_params(axis="y", colors=tau_color)
    ax_curve.set_xlim(phi_deg.min(), phi_deg.max())
    ax_curve.set_xticks([0, 45, 90, 135, 180])
    ax_curve.set_ylim(-1.05, 1.05)               # 2.5% margin, ticks aligned
    ax_curve.set_yticks([-1, 0, 1])
    ax_curve.set_yticklabels(["-1.0", "0.0", "1.0"])

    ax_f = ax_curve.twinx()
    ax_f.patch.set_visible(False)             # don't paint over the tinted panel
    l2, = ax_f.plot(phi_deg, force_curve, color=force_color, lw=3.0, ls="--",
                    label=r"$F_x/F_{max}$", zorder=3)
    ax_f.set_ylabel(r"$F_x/F_{max}$", color=force_color)
    ax_f.tick_params(axis="y", colors=force_color)
    ax_f.set_ylim(-0.025, 1.025)                 # same 2.5% margin -> ticks align
    ax_f.set_yticks([0, 0.5, 1])
    ax_f.set_yticklabels(["0.0", "0.5", "1.0"])
    ...
    # E_pol indicator (linear polarization) in the empty top-centre of the panel
    ax_curve.annotate("", xy=(0.50, 0.82), xytext=(0.34, 0.82),
                      xycoords="axes fraction",
                      arrowprops=dict(arrowstyle="<|-|>", color="crimson",
                                      lw=2.2, mutation_scale=14))
    ax_curve.text(0.42, 0.86, r"$E_{pol}$", transform=ax_curve.transAxes,
                  color="crimson", ha="center", va="bottom",
                  fontsize=15 * fs, zorder=8)
    ...
    ax_lin.set_facecolor(panel_bg)
    _solid_line(ax_lin, xy_lin[:, 0], xy_lin[:, 1], line_color, lw=line_width)
    hx, hy = xr / 2, yr / 2
    inside = np.where((np.abs(xy_lin[:, 0] - cen_lin[0]) <= hx) &
                      (np.abs(xy_lin[:, 1] - cen_lin[1]) <= hy))[0]
    if len(inside):
        e = int(inside[-1])                         # end of the visible run
        ang_e = float(_orientations(xy_lin, th_lin, np.array([e]))[0])
        _place_rotated_image(ax_lin, particle_image, xy_lin[e], img_span,
                             np.degrees(ang_e), image_angle_offset_deg)
    ...
    gx, gy, Lg = cen_lin[0] - 0.12 * xr, cen_lin[1] + 0.14 * yr, 0.08 * xr
    ...
    ax_lin.add_patch(FancyArrowPatch((gx - dd[0], gy - dd[1]),
                                     ...
                                     arrowstyle="<|-|>", mutation_scale=14,
                                     color="crimson", lw=2.2, zorder=8))
    ax_lin.text(gx, gy +0.1 * yr, r"$E_{pol}$", color="crimson",
                ha="center", va="bottom", fontsize=15 * fs, zorder=8)
    # velocity annotation (mirrors the R_T arrow in the circular panel)
    if speed_um_s is not None:
        vy = cen_lin[1] - 0.30 * yr
        vx0 = cen_lin[0] - 0.05 * xr
        Lv = 0.22 * xr
        # ax_lin.add_patch(FancyArrowPatch((vx0, vy), (vx0 + Lv, vy),
        #                                  arrowstyle="-|>", mutation_scale=16,
        #                                  color="black", lw=2.0, zorder=8))
        ax_lin.text(vx0 + 0.5 * Lv, vy + 0.1 * yr,
                    rf"$v \approx {speed_um_s:.1f}\,\mu$m/s",
                    ha="center", va="top", fontsize=14 * fs, zorder=8)

    # ---- right: circular trajectory ----
    ax_cir.set_facecolor(panel_bg)
    _solid_line(ax_cir, xy_cir[:, 0], xy_cir[:, 1], line_color, lw=line_width)
    e = len(xy_cir) - 1                             # end of the orbit
    ang_e = float(_orientations(xy_cir, th_cir, np.array([e]))[0])
    _place_rotated_image(ax_cir, particle_image, xy_cir[e], img_span,
                         np.degrees(ang_e), image_angle_offset_deg)
...
def plot_polarization_trajectories_stacked(
    ...
    gx, gy, Lg = cen_lin[0] - 0.42 * xr, cen_lin[1] + 0.22 * yr, 0.06 * xr
    ...
    ax_lin.add_patch(FancyArrowPatch((gx - dd[0], gy - dd[1]),
                                     ...
                                     arrowstyle="<|-|>", mutation_scale=14,
                                     color="crimson", lw=2.2, zorder=8))
    ax_lin.text(gx, gy + 0.01 * yr, r"$E_{pol}$", color="crimson",
